...
# Patrón E-14 con X (formato visto en formularios reales)
# Formato: X-D-MM-SS-PP-X donde X son marcadores, D=dept, MM=muni, SS=station, PP=puesto
...
def parse_qr_barcode(barcode: str) -> QRData:
    ...
    # Intentar patrón con X (E-14 presidencial)
    # Formato: "X 7-89-84-17 X" → dept=7, muni=89, zone=84, station=17
    match = PATTERN_WITH_X.match(barcode.strip())
    if match:
        result.dept_code = match.group('dept').zfill(2)
        result.muni_code = match.group('muni').zfill(3)
        result.zone_code = match.group('zone').zfill(2)
        result.station_code = match.group('station').zfill(4)
        # Para este formato, la mesa se extrae del nombre del archivo o header
        result.table_number = None
        result.parse_status = QRParseStatus.PARTIAL
        result.confidence = 0.70
        logger.info(f"QR parsed (X format): dept={result.dept_code}, muni={result.muni_code}")
        return result
