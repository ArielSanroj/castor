    ...
    def index_e14_form(
        ...
        if partidos:
            resultados_parts = [
                ...
                f"",
                f"VOTOS POR PARTIDO/CANDIDATO:"
            ]

            # Ordenar por votos (mayor a menor)
            partidos_ordenados = sorted(partidos, key=lambda p: p.get('total_votos', 0), reverse=True)
            ...
            for i, partido in enumerate(partidos_ordenados, 1):
                ...
                resultados_parts.append(f"#{i} {nombre} ({codigo}): {votos:,} votos ({porcentaje:.1f}%)")

                # Agregar candidatos individuales si hay voto preferente
                candidatos = partido.get('votos_candidatos', [])
                if candidatos:
                    for cand in sorted(candidatos, key=lambda c: c.get('votes', 0), reverse=True)[:5]:
                        cand_num = cand.get('candidate_number', '?')
                        cand_votos = cand.get('votes', 0)
        ...
        for partido in partidos:
            ...
            list_type = partido.get('list_type', 'SIN_VOTO_PREFERENTE')
...
Votos por lista: {votos_agrupacion:,}
Total votos: {votos:,}
Porcentaje de mesa: {(votos/total_computado*100) if total_computado > 0 else 0:.2f}%
Confianza OCR: {confidence*100:.1f}%
"""

            # Agregar candidatos si hay
            candidatos = partido.get('votos_candidatos', [])
            if candidatos:
                partido_content += "\nVotos por candidato (preferente):\n"
                for cand in sorted(candidatos, key=lambda c: c.get('votes', 0), reverse=True):
                    cand_num = cand.get('candidate_number', '?')
                    cand_votos = cand.get('votes', 0)
                    partido_content += f"- #{cand_num}: {cand_votos:,} votos\n"
