...
class RegistraduriaDataLoader:
    ...
    async def _extract_departamentos(self, page) -> List[Dict[str, str]]:
        ...
        await page.wait_for_selector('select[name="departamento"], #departamento', timeout=10000)

        departamentos = await page.evaluate("""
            () => {
                const select = document.querySelector('select[name="departamento"], #departamento');
                if (!select) return [];
    ...
    async def _extract_municipios(self, page, departamento_id: str) -> List[Dict[str, str]]:
        ...
        try:
            ...
            await page.select_option('select[name="departamento"], #departamento', departamento_id)
            await asyncio.sleep(1)  # Esperar a que cargue

            # Extraer municipios
            municipios = await page.evaluate("""
                () => {
                    const select = document.querySelector('select[name="municipio"], #municipio');
                    if (!select) return [];
    ...
    async def _extract_zonas(self, page, departamento_id: str, municipio_id: str) -> List[Dict[str, str]]:
        ...
        try:
            ...
            await page.select_option('select[name="municipio"], #municipio', municipio_id)
            await asyncio.sleep(1)

            # Extraer zonas
            zonas = await page.evaluate("""
                () => {
                    const select = document.querySelector('select[name="zona"], #zona');
                    if (!select) return [{ id: null, nombre: null }];  // Algunos municipios no tienen zonas
    ...
    async def _extract_puestos(
        ...
        try:
            ...
            if zona_id:
                await page.select_option('select[name="zona"], #zona', zona_id)
                await asyncio.sleep(0.5)

            # Extraer puestos
            puestos = await page.evaluate("""
                () => {
                    const select = document.querySelector('select[name="puesto"], #puesto');
