...
class E14Worker:
    ...
    async def _navigate_to_e14(self, task: ScrapingTask):
        ...
        corporacion_selector = f"select[name='corporacion']"
        await self.page.select_option(corporacion_selector, task.corporacion)
        await self._random_delay(0.5, 1)

        # Seleccionar departamento
        await self._wait_and_select('departamento', task.departamento_id)
        await self._random_delay(0.5, 1)

        # Seleccionar municipio
        await self._wait_and_select('municipio', task.municipio_id)
        await self._random_delay(0.5, 1)

        # Seleccionar zona si existe
        if task.zona_id:
            await self._wait_and_select('zona', task.zona_id)
            await self._random_delay(0.5, 1)

        # Seleccionar puesto si existe
        if task.puesto_id:
            await self._wait_and_select('puesto', task.puesto_id)
            await self._random_delay(0.5, 1)

        # Hacer clic en consultar/buscar
        submit_button = await self.page.query_selector('button[type="submit"], input[type="submit"], .btn-consultar')
        if submit_button:
    ...
    async def _wait_and_select(self, field_name: str, value: str):
        ...
        selector = f"select[name='{field_name}'], #{field_name}"
        try:
